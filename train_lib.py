...
import advanced_data as advanced_data
from util import CLASS_TO_ID


def train(model, optimizer, train_instances, validation_instances, num_epochs, batch_size, serialization_dir, advanced=False):
    
    if advanced:
      print("\nGenerating train batches")
      train_batches = advanced_data.generate_batches(train_instances, batch_size)
      print("\nGenerating val batches")
      val_batches = advanced_data.generate_batches(validation_instances, batch_size)
    else:
      print("\nGenerating train batches")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_inputs, batch_labels in generator_tqdm:
            with tf.GradientTape() as tape:
                ...
                # Loop over all the trainable parameters
                for train_var in model.trainable_variables:
                  regularization = regularization + (tf.nn.l2_loss(train_var))
                regularization = regularization_lambda * regularization
