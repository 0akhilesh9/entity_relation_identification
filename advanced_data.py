...
def read_instances(data_file_path: str,
                   max_allowed_num_tokens: int = 150, test: bool=False, shortest_path_flag = False) -> List[Dict]:
    """
    ...
    for idx in tqdm(range(0, len(lines), line_inc)):
        ...
        if shortest_path and shortest_path_flag:
            tokens = []
            pos = []
            for token in doc:
        ...
        for i in range(len(doc)):
          if start in str(doc[i]):
            e1_idx = i
          if end in str(doc[i]):
            e2_idx = i

        # Extract position information of words relative to the entities
        position_e1, position_2 = list(), list()
        for idx, word in enumerate(doc):
            # wrt entity1
            position_e1.append(get_position_feature(idx - e1_idx))
            # wrt entity2
            position_2.append(get_position_feature(idx - e2_idx))

        instance["text_tokens"] = tokens
        instance["pos_tags"] = pos
        instance["pos1"] = position_e1
        instance["pos2"] = position_2
...
def get_position_feature(feature_val):
    #Limit the relative postion range to -limit ~ limit i.e., 0 ~ limit * 2+2 
    pos_distance_limit = 50
    if feature_val < -pos_distance_limit:
        return 0
    elif feature_val >= -pos_distance_limit and feature_val <= pos_distance_limit:
        return feature_val + pos_distance_limit + 1
    else:
        return pos_distance_limit * 2 + 2
...
def generate_batches(instances: List[Dict], batch_size) -> List[Dict[str, np.ndarray]]:
    ...
    for batch_of_instances in tqdm(batches_of_instances):
        ...
        batch = {"inputs": np.zeros((count, max_num_token_ids), dtype=np.int32),
                 ...
                 "pos1": np.zeros((count, max_num_token_ids), dtype=np.int32), 
                 "pos2": np.zeros((count, max_num_token_ids), dtype=np.int32)}
        ...
        for batch_index, instance in enumerate(batch_of_instances):
            ...
            # position feature for the entity1
            pos1 = np.array(instance["pos1"])
            batch["pos1"][batch_index][:num_tokens] = pos1
            # position feature for the entity2
            pos2 = np.array(instance["pos1"])
            batch["pos1"][batch_index][:num_tokens] = pos2
